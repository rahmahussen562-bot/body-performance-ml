...
SELECT 
    ...
    G.Email, 
    G.City,
    COUNT(CASE WHEN R.reservation_status = 'checked_out' THEN 1 END) AS TotalStays,
...
LEFT JOIN 
    Invoices I ON R.ReservationID = I.ReservationID
GROUP BY 
    G.GuestID, 
    G.FullName, 
    G.NationalID_Passport, 
    ...
    G.Email, 
    G.City;
GO -- هذه الكلمة ضرورية جداً للفصل بين تعريف الـ View وبين استخدامها

-- ثانياً: الآن يمكنك الاستعلام من الـ View التي أنشأتها
SELECT TOP 20 
    GuestID, 
    FullName, 
    NationalID_Passport, 
    ...
    LifetimeSpend
FROM vw_GuestMaster
ORDER BY LifetimeSpend DESC; -- تم تصحيح الخطأ الإملائي هنا (حذف حرف D)
...
-- 1. حذف الـ View إذا كانت موجودة مسبقاً لتجنب الأخطاء
IF OBJECT_ID('vw_GuestMaster', 'V') IS NOT NULL
    DROP VIEW vw_GuestMaster;
GO

-- 2. إنشاء الـ View بشكل صحيح
CREATE VIEW vw_GuestMaster AS
SELECT 
    G.GuestID, 
    G.FullName, 
    ...
    G.Email, 
    G.City,
    -- حساب عدد الإقامات المكتملة فقط
    COUNT(R.ReservationID) AS TotalReservations,
    -- حساب إجمالي المبالغ المدفوعة من جدول الفواتير
    ISNULL(SUM(I.PaidAmount), 0) AS LifetimeSpend
FROM 
    Guests G
LEFT JOIN 
...
-- 3. تجربة الاستعلام من الـ View مع ترتيب النتائج (أعلى 20 عميل إنفاقاً)
SELECT TOP 20 
    GuestID, 
    FullName, 
    TotalReservations, 
...
SELECT 
    ...
    R.BuildingID,
    R.AvailabilityStatus AS InternalStatus -- الحالة العامة (Available/Maintenance)
FROM 
    Rooms R
JOIN 
...
-- حدد التواريخ التي تريد البحث فيها هنا
...
SELECT 
    ...
    RoomType, 
    NightlyRate,
    -- سنعرض الحالة بناءً على البحث
    CASE 
        WHEN InternalStatus = 'Maintenance' THEN 'Maintenance'
        ELSE 'Available' 
    END AS AvailabilityStatus
FROM vw_RoomAvailability
WHERE InternalStatus <> 'Maintenance' -- استبعاد الغرف التي تحت الصيانة تماماً
AND RoomID NOT IN (
    -- استبعاد الغرف التي لها حجز متداخل مع الفترة المطلوبة
    SELECT RoomID 
    FROM Reservations 
    WHERE reservation_status IN ('confirmed', 'checked_in') -- الحجوزات النشطة فقط
    AND check_in_date < @OutDate  -- تاريخ دخول الحجز القديم قبل خروج العميل الجديد
    AND check_out_date > @InDate  -- تاريخ خروج الحجز القديم بعد دخول العميل الجديد
)
